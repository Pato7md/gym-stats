...
def dashboard():
    ...
    # Fallback HTML für initiale Seite (wird durch JS überschrieben)
...
def api_geraete_options():
    ...
    geraete_options = get_geraete_options(df, person, gym, start, end) or []
    return jsonify(geraete_options)
...
def api_gym_table():
    ...
    try:
        # Wenn keine Geräte übergeben → alle im Zeitraum nehmen
        if not geraete:
        ...
        daily = (
            filtered_df.groupby(['gerät', 'datum'], as_index=False)
                       .agg(
                           volumen_sum=('vol_gesamt', 'sum'),
        ...
        agg = (
            daily.groupby('gerät', as_index=False)
                 .agg(
                     anzahl_besuche=('datum', 'nunique'),
        ...
        # Rundungen: Volumen 0, Gewicht 1, Wdh 1
        ...
        table = agg.rename(columns={
            'gerät': 'Gerät',
            'anzahl_besuche': 'Anzahl Besuche',
            'avg_volumen': 'Volumen / Besuch',
            'avg_gewicht': 'Gewicht / Besuch',
            'avg_wdh': 'Wdh / Besuch',
        }).to_dict('records')

        # Achtung: Dein partial_table.html sollte <tr data-geraet="{{ row['Gerät'] }}"> setzen!
...
def api_gym_plot():
    ...
    geraete = request.args.getlist('geraete')  # darf leer sein!

    # Prüfen, ob Grundparameter gesetzt
    if not all([person, gym, start, end, metric]):
        return jsonify({'error': 'Fehlende Parameter'}), 400

    # Wenn keine Geräte ausgewählt → kein Diagramm anzeigen
    if not geraete:
        return ('', 204)
    ...
    agg, fig = get_aggregated_data(filtered_df, metric)
    if not fig or len(fig.data) == 0:
        return ('', 204)

    # Punkte auch bei nur einem Besuch, nur Datum in X-Achse
